...
class LTAS_FC_MNIST(bcnn.Module):
    def __init__(self):
        super(LTAS_FC_MNIST, self).__init__()
        # global flow
    ...
    def fit(self, data, label, criteria, optimizer):
        # local flow update
        optimizer.zero_grad()
        local_output = self.forward(data, get_local_output=True)
        local_loss = criteria[0](local_output, label)
        local_loss.backward()
        optimizer.step()
...
class LTAS_FC_CIFAR10(bcnn.Module):
    def __init__(self):
        super(LTAS_FC_CIFAR10, self).__init__()
        # global flow
    ...
    def forward(self, x, get_local_output=False):
        if get_local_output:
            with torch.no_grad():
                ...
                x = self.fc2(x1)
                x2 = self.relu2(x)
            return self.lfc1(x1), self.lfc2(x2)
    ...
    def fit(self, data, label, criteria, optimizer):
        # local flow update
        optimizer.zero_grad()
        local_output1, local_output2 = self.forward(data, get_local_output=True)
        local_loss1 = criteria[0](local_output1, label)
        local_loss2 = criteria[0](local_output2, label)
...
class LTAS_FC_CIFAR100(bcnn.Module):
    def __init__(self):
        super(LTAS_FC_CIFAR100, self).__init__()
        # global flow
        self.fc1 = nn.Linear(3*32*32, 4096)
        self.relu1 = nn.ReLU()
